...
PECTORAL::PECTORAL(uint8_t id1, uint8_t id2, uint8_t id3, uint8_t id4, XH430 *dxl) : led(LED3)
{
    id1_ = id1;
    id2_ = id2;
    id3_ = id3;
...
void PECTORAL::set_profile(int8_t count, float amplitude,
...
void PECTORAL::pectoral_func(void)
{
    if (cont_ == 1)
    {
        if (direction_ == 0)
...
void PECTORAL::set_tick(uint8_t onoff, microseconds wait_time)
{
...
void PECTORAL::set_cont(uint8_t cont)
{
    cont_ = cont;
}

void PECTORAL::set_direction(uint8_t direction)
{
    direction_ = direction;
}

void PECTORAL::set_period(float period)
{
    period_ = period;
